...
const CoCreateTemplate = {
	...
	ioInstance: null, /** intersection observer instance **/
	...
	__initSocketEvent: function() {
		...
		CoCreateSocket.listen('readCollectionList', function(data) {
			...
			let item = CoCreateFilter.getObjectByFilterId(self.items, item_id);
		...
		for (let k = 0; k < items.length; k++) {
			...
			for (let i = 0; i < displayList.length; i++) {
				...
				if (passTo && passTo == pass_id) {
					...
					this.__setCommonValue(display, items[k][name] || id)
				}
			}
	...
	// __registerElementIntoObseve: function(item) {
	// 	let templateId = item.templateId
	// 	let element = document.querySelector(`[data-template_id="${templateId}"][data-fetch_type="scroll"]`)
	// 	if (element) {
	// 		this.ioInstance.observe(element)
	// 	}
	// },
...
const CoCreateRender = {
	...
	customFillElementTemplate: function(element,row,attr_name,value,type){
	
	},
	eachChildren:function(els,row, type, template){
		console.log(row)
		let that = this;
		els.forEach(e=>{
			if(e.children.length>0)
				that.eachChildren(Array.from(e.children), row, type, template)
			Array.from(e.attributes).forEach(attr=>{
				...
				if(value != false){
					...
					this.customFillElementTemplate(e,template,row,attr.value,value,type);
				}
	...
	template_id : function(type,dataResult) {
	 try{
		 ...
		 dataResult.forEach(row=>{
			 ...
			 that.eachChildren(Array.from(template.children),row,type,template);
