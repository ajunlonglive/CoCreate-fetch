...
function initEngines(container) {
  ...
  for (let i=0; i < templateWrappers.length; i++) {
    ...
    let filter = CoCreateFilter.setFilter(templateWrapper, "data-template_id", "template");
    if (!filter) continue;
    ...
    templateWrapper.addEventListener("changeFilterInput", function(e) {
      ...
      CoCreateFilter.fetchData(eObj.filter);
    })
...
function initTemplateByElement(element, isInit) {
  ...
  let tempalteObj = CoCreateFilter.getObjectByFilterId(eOBJs, template_id);
  let filter = null;
  ...
  if (!tempalteObj) {
    ...
    element.addEventListener("changeFilterInput", function(e) {
      ...
      CoCreateFilter.fetchData(tempalteObj.filter);
    })
  } else {
...
function fetchedEngine(data) {
  ...
  let eObj = CoCreateFilter.getObjectByFilterId(eOBJs, eId);
...
function createdDocumentForEngine(data) {
  ...
  for (let i=0; i < eOBJs.length; i++) {
    ...
    if (ids.length > 0) {
      let info = CoCreateFilter.makeFetchOptions(eObj.item);
